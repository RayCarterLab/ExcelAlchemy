"""Workbook-coordinate issue maps and API-facing issue summaries."""

from collections.abc import Iterable
from dataclasses import dataclass

from excelalchemy.errors import ExcelCellError, ExcelRowError
from excelalchemy.primitives.identity import ColumnIndex, Label, RowIndex

type RowIssue = ExcelRowError | ExcelCellError


def row_number_for_humans(row_index: RowIndex) -> int:
    return int(row_index) + 1


def column_number_for_humans(column_index: ColumnIndex) -> int:
...
class CellIssueRecord:
    ...
    def to_dict(self) -> dict[str, object]:
        ...
        payload['row_number_for_humans'] = row_number_for_humans(self.row_index)
        payload['column_index'] = int(self.column_index)
        payload['column_number_for_humans'] = column_number_for_humans(self.column_index)
...
class CellErrorMap(dict[RowIndex, dict[ColumnIndex, list[ExcelCellError]]]):
    ...
    def row_numbers_for_humans(self) -> tuple[int, ...]:
        return tuple(row_number_for_humans(row_index) for row_index in self.row_indices())
    ...
    def column_numbers_for_humans(self) -> tuple[int, ...]:
        return tuple(column_number_for_humans(column_index) for column_index in self.column_indices())
        ...
        for unique_label, records in grouped.items():
            ...
            summaries.append(
                FieldIssueSummary(
                    ...
                    row_numbers_for_humans=tuple(row_number_for_humans(index) for index in row_indices),
    ...
    def summary_by_row(self) -> tuple[RowIssueSummary, ...]:
        ...
        for row_index, records in grouped.items():
            summaries.append(
                RowIssueSummary(
                    row_index=row_index,
                    row_number_for_humans=row_number_for_humans(row_index),
                    error_count=len(records),
                    codes=tuple(sorted({record.error.code for record in records})),
